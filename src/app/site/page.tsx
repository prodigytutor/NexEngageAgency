  ...
  return (
    <>
        ...
        <section className="flex justify-center items-center flex-col gap-4  md:!mt-20 mt-[-40px]">
          <h2 className="text-4xl text-center">Choose whats fits you</h2>
          <p className="text-muted-foreground text-center">
              Our straightforeard pricing plans are tailored to meet your needs. if {"you`re"} not <br/> 
                ...
                <CardFooter className="flex flex-col items-start gap-4">
                  <div>
                      {card.features.map((feature)=>
                        <div key={feature} className="flex gap-2 items-cneter">
                            <Check/>
                            <p>{feature}</p>
                        </div>
                      )}
                  </div>
                  <Link className={clsx("w-full text-center bg-primary p-2 rounded-md", {"!bg-muted-foreground" : card.title !== "Unlimited Saas"})} href={`./agency?plan=${card.priceId}`}>
                    Get Started
                  </Link>
                </CardFooter>
              </Card>
